...
- 정확한 안내나 문제 파악을 위한 '목적이 분명한' 확인 질문(예: "언제 요청하셨나요?", "어떤 상황에서 발생했나요?", 발생 시점·상황 구체화 질문)은 지침이 권장하는 '확인 질문 품질'의 핵심이며 가점 요소입니다. 이런 질문을 "고객이 부담스러울 수 있다"는 식으로 약점으로 지적하면 안 됩니다. 오직 같은 정보를 불필요하게 여러 번 되묻거나, 목적 없이 과도하게 확인을 요구해 고객을 지치게 하는 경우에만 약점으로 보세요.
- 점수는 후하게 주세요. 고객 문제를 이해하려 질문하고 정리하며 상담 흐름을 유지했다면 기본적으로 높은 점수를 주고, 명확히 응대 품질을 떨어뜨린 문제가 있을 때만 감점하세요.
...
[약점·리스크]
- 여기에는 실제로 응대 품질을 '명확히' 떨어뜨린 문제만 적습니다. 칸을 채우려고 사소하거나 추측성 약점을 지어내지 마세요 — 약점을 억지로 만드는 것은 평가 오류입니다. 좋은 상담이면 이 칸이 비어 있는 게 정상이며, 그럴 때는 다른 말을 덧붙이지 말고 반드시 "특별한 약점 없음"이라고만 적으세요.
- 다음은 약점이 아니니 절대 적지 마세요: 목적이 분명한 확인 질문, 제품·서비스·절차를 모르는 것, 상담 중에 문제를 완전히 해결하지 못한 것, "더 ~했으면 좋았을 것"류의 사소한 아쉬움.
