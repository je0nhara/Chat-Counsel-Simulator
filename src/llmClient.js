...
[지원자에게 안내된 표준 처리 프로세스]
${scenario.process || "(별도 안내 없음)"}

[평가 관점]
- 지원자는 위 '표준 처리 프로세스'를 안내받은 상태에서 응대했습니다. 이 프로세스를 적절히 활용해 고객에게 안내했는지를 봅니다(단, 토씨 하나까지 외워 말할 필요는 없고 핵심을 자연스럽게 전달했으면 충분합니다).
- 동시에 응대 태도가 핵심입니다: 공감·경청, 고객 감정·니즈 파악, 정중한 태도, 적절한 확인·질문(소통), 읽기 쉬운 문장(맞춤법·가독성), 깔끔한 마무리.
- 절차를 모른다고 무작정 "확인 후 연락드리겠다"로만 끝내기보다, 안내된 프로세스를 바탕으로 진행 상황·예상 시점 등을 설명했는지 함께 보세요.
